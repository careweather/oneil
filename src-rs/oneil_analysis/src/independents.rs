...
use crate::output::error::IndependentsErrors;
...
/// whose dependency set is empty is recorded as an independent. Then recurses into each
/// reference (evaluated submodel) and merges their independents into the result.
///
/// Returns a map from each model path to the set of parameter names that have no
/// builtin, parameter, or external dependencies, plus an [`IndependentsErrors`] of model paths
/// that had evaluation errors.
#[must_use]
pub fn get_independents<E: ExternalAnalysisContext>(
    ...
    let independents: IndexSet<String> = model
        .parameters
        .iter()
    ...
    for ref_path in model.references.values() {
        let (nested_map, nested_errors) = get_independents(ref_path.as_path(), external_context);

        for (path, params) in nested_map {
            result.insert(path, params);
        }
