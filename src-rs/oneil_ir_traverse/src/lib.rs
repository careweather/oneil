...
//! their components (python imports, submodels, parameters, tests, and submodel tests)
...
/// * `test_process` - Processor for tests
...
pub fn traverse<PythonImportP, SubmodelP, ParameterP, TestP, SubmodelTestP>(
        ...
        .fold(model_map_builder, |builder, model_path| {
            ...
            let builder =
                model
                    .get_tests()
                    .into_iter()
                    .fold(builder, |mut builder, (test_index, test)| {
                        // process the test
                        let result = test_process.process(test_index, test);

                        // add the test to the model map
                        match result {
                            Ok(output) => builder.add_test_data(
                                model_path.clone(),
                                test_index.clone(),
                                output,
                            ),
                            Err(error) => builder.add_test_error(
                                model_path.clone(),
                                test_index.clone(),
                                error,
                            ),
