...
impl PythonPath {
    ...
    ///
    /// ```rust
    /// use oneil_module::reference::PythonPath;
    /// use std::path::PathBuf;
    ///
    ...
    ///
    /// // Path with .py extension is preserved
    /// let path2 = PythonPath::new(PathBuf::from("numpy/random.py"));
    /// assert_eq!(path2.as_ref().to_string_lossy(), "numpy/random.py");
    ///
