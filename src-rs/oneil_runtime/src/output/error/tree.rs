...
impl TreeError {
    ...
    /// [`Parse`](TreeError::Parse). If `self` is [`TreeErrors`](TreeError::TreeErrors)
    ...
    pub fn insert_parameter_errors(&mut self, name: String, errors: Vec<OneilError>) {
        if let TreeError::TreeErrors { parameter_errors } = self {
            parameter_errors.entry(name).or_insert(errors);
        }
    }

    /// Inserts all errors from `other` into `self`. If `other` is a
    /// [`Parse`](TreeError::Parse), `self` is replaced with it. If both are
    /// [`TreeErrors`](TreeError::TreeErrors), entries from `other` are merged
    /// in; for parameters already present in `self`, the existing errors are
    /// kept and `other`'s errors are not added.
    pub fn insert_all(&mut self, other: TreeError) {
        match (self, other) {
            (self_ref, other @ TreeError::Parse(_)) => *self_ref = other,
            (TreeError::Parse(_), _) => {}
            (
                TreeError::TreeErrors { parameter_errors },
                TreeError::TreeErrors {
